...
\subsection{Plage d’adressage privé}
...
\subsection{Gestion de la plage publique}
...
\subsection{Gestion de la plage privée}
...
\subsection{Réseau privé Cloud (Intranet) (LAN\_3-31)}
	...
	\subsection{Réseau Publique Cloud (Internet) (Lan\_3-32)}
...
\chapter{Mise en place des services réseau : DHCP, NAT et ACL}
...
%=================================================
	\section{Attribution dynamique des adresses IP : DHCP}
	
		\subsection{Choix du mode d’adressage}
	...
	\subsection{Implémentation du service DHCP}
	...
	La mise en place du DHCP est implémentée sur les routeurs, en mode de configuration globale (configure terminal) et elle repose sur les étapes suivantes: 
		...
		\paragraph{i. Création des pools DHCP\\}
	...
	\paragraph{iv. Implémentation du DHCP sur les équipements \\}
...
\section{Traduction d’adresses : NAT}
...
\subsection{Découpage des plages publiques}
...
\subsection{Validation du fonctionnement du NAT}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3.5cm}|c|l|}
		...
		Routage retour& \texttt{123.1.1.160 } & Ajout d’une route  \\
...
\subsection{Expression des besoins en matière d’ACL de sécurité}
...
\subsection{Schéma des flux autorisés et interdits}
...
\begin{itemize}
	...
	\end{itemize}
\end{itemize}
\subsection{Structuration des ACL}
...
\subsection{Configuration des ACL de sécurité}
...
\begin{figure}[]
	\centering
	\includegraphics[width=0.9\textwidth]{../captures/img2.png}
...
\begin{itemize}
	\item \textbf{Routeurs de cœur :} quatre routeurs (RO1 à RO4) interconnectés entre eux selon une topologie maillée. Cette interconnexion assure une redondance des chemins et une meilleure résilience du réseau.
	
	\item \textbf{Routeurs de périphérie :} trois routeurs (RO11, RO21 et RO31) assurant l’accès aux réseaux externes. Ces routeurs permettent de connecter les différents sites et services au réseau opérateur via des liens d’accès dédiés.
\end{itemize}
...
\begin{itemize}
	\item un \textbf{cœur de réseau}, responsable du transport des données ;
	\item une \textbf{périphérie}, assurant l’accès aux différents réseaux clients et services.
\end{itemize}
...
\vspace{0.5cm}
\textbf{Remarque :}
Dans le cadre de cette implémentation, une image spécifique de routeur a été utilisée pour l’ensemble des équipements du réseau opérateur. Il s’agit de l’image \texttt{c7200-adventerprisek9-mz.124-24.T5}, importée dans l’environnement GNS3 via le fichier \texttt{cisco-7200-edge.gns3a}.
...
\subsection{Routage dynamique via le protocole OSPF (routeurs du backbone)}

Le routage dynamique au sein du réseau opérateur est assuré à l’aide du protocole \textbf{OSPF} (\textit{Open Shortest Path First}). 
...
\section{Validation du fonctionnement du protocole OSPF}
...
\subsection{Wiresharque (Hello \& LS Update \& voisinage)}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../captures/10_ospf_lsupdate.png}
...
\end{figure}
Une fois ces adjacences établies, les routeurs échangent leur base de données de topologie, permettant la construction d’une vision cohérente du réseau. L’absence de messages de type \textit{LS Update} dans les captures s’explique par le fait que le réseau est dans un état stable, sans modification de topologie.\\
\textbf{Remarque : } pour visualiser un \textit{LS Update}, on peut le provoquer ainsi : 
\begin{verbatim}
	RO11(config)#shutdown interface e1/0
	RO11(config)#no shutdown
\end{verbatim}

\subsection{Tests de la connectivité du réseau opérateur}
...
\begin{enumerate}
	\item  entre toutes paires de routeurs des réseaux extrémités : RE1, RE2 et RF2,
